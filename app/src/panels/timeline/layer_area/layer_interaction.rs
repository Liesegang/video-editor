...
use super::layers::{calculate_insert_index, calculate_layer_rect, draw_waveform};
...
/// Returns `u64::MAX` when the source has no known duration (infinite extension).
pub(super) fn compute_source_max_out_frame(
    ...
    if let Some(duration) = duration_frame {
        ...
        if source_end_offset > 0 {
            ...
            // fall back to the current out_frame so the layer stays valid.
            out_frame
        }
    } else {
        u64::MAX
...
/// Clamps to `[0, out_frame - 1]` so the layer keeps at least 1 frame.
...
/// Compute new (in_frame, out_frame) when dragging a layer horizontally.
/// Preserves layer duration and clamps so `in_frame >= 0`.
...
#[derive(Debug)]
pub(super) enum DeferredLayerAction {
    /// Update layer time (resize)
    UpdateLayerTime {
        source_id: Uuid,
        new_in_frame: u64,
        new_out_frame: u64,
    },
    /// Move layer to track at index (reorder/move)
    MoveLayerToTrack {
        comp_id: Uuid,
    ...
    RemoveLayer { track_id: Uuid, source_id: Uuid },
    /// Push history state after changes
    PushHistory,
}

// Helper to find which track contains a source (looking inside Layer containers)
pub(super) fn find_track_containing_source(
    project: &Project,
    root_track_ids: &[Uuid],
    source_id: Uuid,
) -> Option<Uuid> {
    fn search_track(project: &Project, track_id: Uuid, source_id: Uuid) -> Option<Uuid> {
        if let Some(track) = project.get_track(track_id) {
            for child_id in &track.child_ids {
                ...
                match project.get_node(*child_id) {
                    Some(Node::Track(_)) => {
                        if let Some(found) = search_track(project, *child_id, source_id) {
                            return Some(found);
                        }
                    }
                    Some(Node::Layer(layer)) => {
                        // Source lives inside a Layer container — return the parent Track
                        if layer.child_ids.contains(&source_id) {
                            return Some(track_id);
                        }
                    }
                    _ => {}
    ...
    for root_id in root_track_ids {
        if let Some(found) = search_track(project, *root_id, source_id) {
            return Some(found);
        }
    }
    None
...
pub(super) fn draw_single_layer(
    ui_content: &mut Ui,
    content_rect: egui::Rect,
    editor_context: &mut EditorContext,
    deferred_actions: &mut Vec<DeferredLayerAction>,
    ...
    let (r, g, b) = source.display_color();
    let layer_color = egui::Color32::from_rgb(r, g, b);
    ...
    if let Some((dragged_id, r_track_id, src_idx, dst_idx, header_row_idx)) = reorder_state {
        if source.id == *dragged_id {
            ...
            if let Some(DisplayRow::SourceRow { child_index, .. }) = display_rows.get(row_index) {
    ...
    let initial_layer_rect = calculate_layer_rect(
        source.in_frame,
        source.out_frame,
        visual_row_index,
    ...
    let safe_width = initial_layer_rect.width();

    // Visibility Culling
    if !content_rect.intersects(initial_layer_rect) {
        return;
    }

    // --- Interaction for layers ---
    let sense = if is_summary_layer {
        egui::Sense::click()
    } else {
        egui::Sense::click_and_drag()
    };

    let interaction_id = if is_summary_layer {
        egui::Id::new(source.id).with("summary").with(row_index)
    } else {
        egui::Id::new(source.id)
    };

    let layer_resp = ui_content.interact(initial_layer_rect, interaction_id, sense);
    ...
    if !is_summary_layer {
        layer_resp.context_menu(|ui| {
            ...
            let menu = ContextMenuBuilder::new()
                .danger_action(icons::TRASH, "Remove", LayerAction::Remove)
                .build();
            if let Some(action) = show_context_menu(ui, &menu) {
                match action {
                    LayerAction::Remove => {
                        if let Some(_comp_id) = editor_context.selection.composition_id {
                            deferred_actions.push(DeferredLayerAction::RemoveLayer {
                                track_id: track.id,
                                source_id: source.id,
                            });
    ...
    if !is_summary_layer {
        let left_edge_rect = egui::Rect::from_min_size(
            egui::pos2(initial_layer_rect.min.x, initial_layer_rect.min.y),
            egui::vec2(EDGE_DRAG_WIDTH, initial_layer_rect.height()),
        );
        left_edge_resp = Some(ui_content.interact(
            left_edge_rect,
            egui::Id::new(source.id).with("left_edge"),
            egui::Sense::drag(),
        ));

        let right_edge_rect = egui::Rect::from_min_size(
            egui::pos2(
                initial_layer_rect.max.x - EDGE_DRAG_WIDTH,
                initial_layer_rect.min.y,
            ),
            egui::vec2(EDGE_DRAG_WIDTH, initial_layer_rect.height()),
        );
        right_edge_resp = Some(ui_content.interact(
            right_edge_rect,
            egui::Id::new(source.id).with("right_edge"),
            egui::Sense::drag(),
        ));
    }
    ...
    if let (Some(left), Some(right)) = (&left_edge_resp, &right_edge_resp) {
        if left.drag_started() || right.drag_started() {
            ...
            editor_context.select_source(source.id, track.id);
            _is_resizing = true;
        }
    }

    if editor_context.interaction.timeline.is_resizing_entity
        && editor_context.selection.last_selected_entity_id == Some(source.id)
        && !is_summary_layer
    {
        if let (Some(left), Some(right)) = (&left_edge_resp, &right_edge_resp) {
            let mut new_in_frame = source.in_frame;
            let mut new_out_frame = source.out_frame;
            ...
            if new_in_frame != source.in_frame || new_out_frame != source.out_frame {
                if let (Some(_comp_id), Some(_tid)) = (
                    editor_context.selection.composition_id,
                    editor_context.selection.last_selected_track_id,
                ) {
                    deferred_actions.push(DeferredLayerAction::UpdateLayerTime {
                        source_id: source.id,
                        new_in_frame,
                        new_out_frame,
                    });
    ...
    if let (Some(left), Some(right)) = (&left_edge_resp, &right_edge_resp) {
        if left.drag_stopped() || right.drag_stopped() {
            ...
            deferred_actions.push(DeferredLayerAction::PushHistory);
        }
    }

    // Calculate display position
    let mut display_x = initial_layer_rect.min.x;
    let display_y = initial_layer_rect.min.y;

    if editor_context.is_selected(source.id) && layer_resp.dragged() && !is_summary_layer {
        display_x += layer_resp.drag_delta().x;
    }

    let drawing_layer_rect = egui::Rect::from_min_size(
        egui::pos2(display_x, display_y),
        egui::vec2(safe_width, row_height),
    );

    // --- Drawing ---
    let is_sel_entity = editor_context.is_selected(source.id);
    ...
    painter.rect_filled(drawing_layer_rect, 4.0, transparent_color);

    // Draw Audio Waveform
    if (source.kind == SourceKind::Audio || source.kind == SourceKind::Video) && safe_width > 10.0 {
    ...
    // Text label
    let mut layer_text = source.kind.to_string();
    if is_summary_layer {
        layer_text = format!("(Ref) {}", layer_text);
    }

    painter.text(
        drawing_layer_rect.min + egui::vec2(5.0, 5.0),
    ...
    if !editor_context.interaction.timeline.is_resizing_entity && layer_resp.clicked() {
        ...
        match action {
            crate::widgets::selection::ClickAction::Select(id) => {
                editor_context.select_source(id, track.id);
            }
    ...
    if !editor_context.interaction.timeline.is_resizing_entity && layer_resp.drag_started() {
        if !editor_context.is_selected(source.id) {
            editor_context.select_source(source.id, track.id);
        }
        if !is_summary_layer {
            editor_context.selection.last_selected_entity_id = Some(source.id);
    ...
    if !editor_context.interaction.timeline.is_resizing_entity
        && layer_resp.dragged()
        && editor_context.is_selected(source.id)
        && !is_summary_layer
    {
        if layer_resp.drag_delta().length_sq() > 0.0 {
            editor_context.interaction.timeline.dragged_entity_has_moved = true;
        }

        let dt_frames_f32 = layer_resp.drag_delta().x / pixels_per_unit * composition_fps as f32;
        let dt_frames = dt_frames_f32.round() as i64;
        ...
        if dt_frames != 0 {
            if let Some(_comp_id) = editor_context.selection.composition_id {
                ...
                for entity_id in selected_ids {
                    // Use Project.get_source for lookup
                    if let Some(c) = project.get_source(entity_id) {
                        // Find which track contains this source
                        if let Some(_tid) =
                            find_track_containing_source(project, root_track_ids, entity_id)
                        {
        ...
        && !is_summary_layer
    {
        ...
        if let (Some(original_track_id), Some(hovered_track_id), Some(comp_id)) = (
            ...
            deferred_actions.push(DeferredLayerAction::MoveLayerToTrack {
                comp_id,
                original_track_id,
                source_id: source.id,
...
mod tests {
    ...
    #[test]
    fn resize_left_shrinks_layer() {
        // Layer at frames 10-50, drag right by 5 → new in_frame = 15
        let result = compute_resize_left_frame(10, 50, 5);
        assert_eq!(result, 15);
    }

    #[test]
    fn resize_left_extends_layer() {
        // Layer at frames 10-50, drag left by 5 → new in_frame = 5
        let result = compute_resize_left_frame(10, 50, -5);
        assert_eq!(result, 5);
    }
    ...
    fn resize_left_clamps_to_zero() {
        // Layer at frames 10-50, drag left by 20 → clamped to 0
    ...
    fn resize_left_clamps_to_out_minus_one() {
        // Layer at frames 10-50, drag right by 100 → clamped to 49 (out-1)
    ...
    fn resize_left_preserves_minimum_one_frame() {
        // Layer at frames 10-11 (1 frame), drag right by 0 → stays at 10
    ...
    #[test]
    fn resize_right_extends_layer() {
        // Layer at frames 10-50, drag right by 10 → new out_frame = 60
        let result = compute_resize_right_frame(10, 50, 10, u64::MAX);
        assert_eq!(result, 60);
    }

    #[test]
    fn resize_right_shrinks_layer() {
        // Layer at frames 10-50, drag left by 10 → new out_frame = 40
        let result = compute_resize_right_frame(10, 50, -10, u64::MAX);
        assert_eq!(result, 40);
    }
    ...
    fn resize_right_clamps_to_in_plus_one() {
        // Layer at frames 10-50, drag left by 100 → clamped to 11 (in+1)
    ...
    fn resize_right_clamps_to_source_max() {
        // Layer at frames 10-50, drag right by 100 → clamped to source max 80
    ...
    fn drag_preserves_duration() {
        // Layer at frames 10-20 (duration=10), drag right by 5
    ...
    fn drag_left_clamps_at_zero_preserves_duration() {
        // BUG FIX: Layer at frames 10-20 (duration=10), drag left by 100
        // Old code: new_in=0, new_out=max(-80, 0)=0 → ZERO-LENGTH LAYER!
    ...
    fn drag_to_exact_zero() {
        // Layer at frames 10-20, drag left by exactly 10
    ...
    #[test]
    fn drag_single_frame_layer_preserves_duration() {
        // 1-frame layer at frame 5-6
    ...
    // ── Domain: find_track_containing_source ──
    ...
    fn make_test_project() -> (Project, Uuid, Uuid, Uuid, Uuid) {
        ...
        track.id = track_id;
        track.child_ids = vec![source_id_1, source_id_2];

        let source1 = SourceData::new(
            source_id_1,
            None,
            SourceKind::Video,
            0,
        ...
        project.nodes.insert(source_id_1, Node::Source(source1));
        project.nodes.insert(source_id_2, Node::Source(source2));

        (project, track_id, source_id_1, source_id_2, Uuid::new_v4())
    }

    #[test]
    fn find_source_in_direct_track() {
        let (project, track_id, source_id_1, _, _) = make_test_project();
        let result = find_track_containing_source(&project, &[track_id], source_id_1);
        assert_eq!(result, Some(track_id));
    }

    #[test]
    fn find_source_not_in_any_track() {
        let (project, track_id, _, _, _) = make_test_project();
        let missing_id = Uuid::new_v4();
        let result = find_track_containing_source(&project, &[track_id], missing_id);
        assert_eq!(result, None);
    }

    #[test]
    fn find_source_in_nested_track() {
        let (mut project, track_id, _, source_id_2, _) = make_test_project();

        // Create a sub-track and move source_id_2 into it
        let sub_track_id = Uuid::new_v4();
        let mut sub_track = TrackData::new("Sub Track");
        sub_track.id = sub_track_id;
        sub_track.child_ids = vec![source_id_2];

        // Remove source_id_2 from main track, add sub_track instead
        if let Some(Node::Track(t)) = project.nodes.get_mut(&track_id) {
            t.child_ids.retain(|id| *id != source_id_2);
            t.child_ids.push(sub_track_id);
        }
        project.nodes.insert(sub_track_id, Node::Track(sub_track));

        let result = find_track_containing_source(&project, &[track_id], source_id_2);
        assert_eq!(result, Some(sub_track_id));
    }

    #[test]
    fn find_source_with_empty_root_tracks() {
        let (project, _, source_id_1, _, _) = make_test_project();
        let result = find_track_containing_source(&project, &[], source_id_1);
        assert_eq!(result, None);
    }
    ...
    fn drag_then_move_should_preserve_horizontal_position() {
        // Simulate the scenario where UpdateLayerTime and MoveLayerToTrack both
        // fire in the same frame. UpdateLayerTime runs first (horizontal drag)
        // then MoveLayerToTrack should use the UPDATED in_frame, not the stale one.
        //
        // Layer starts at frames 10-20 (duration=10).
        // Horizontal drag moves it to 15-25.
        // MoveLayerToTrack should use in_frame=15 (current), not 10 (stale).
        ...
        // After UpdateLayerTime sets (15, 25), MoveLayerToTrack should read 15
    ...
    fn drag_far_left_then_move_preserves_clamped_position() {
        // Layer at frames 100-200 (duration=100). Drag far left by -500.
        let (new_in, new_out) = compute_drag_frames(100, 200, -500);
        assert_eq!(new_in, 0);
        assert_eq!(new_out, 100);

        // MoveLayerToTrack should use 0 (current), not 100 (stale).
    ...
    #[test]
    fn resize_left_on_layer_starting_at_zero() {
        // Layer at frames 0-30, try to extend left (impossible, already at 0)
        let result = compute_resize_left_frame(0, 30, -10);
        assert_eq!(result, 0);
    }

    #[test]
    fn resize_right_on_one_frame_layer() {
        // Layer at frames 10-11 (1 frame), try to shrink right (impossible)
