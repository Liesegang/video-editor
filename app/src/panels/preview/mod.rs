...
use crate::context::context_types::PreviewTool;
use crate::widgets::viewport::{ViewportConfig, ViewportController, ViewportState};
use library::project::property::Vec2;
        ...
        if let Some(comp) = editor_context.get_current_composition(&proj_read) {
...

            {
                ...
                let region = if region_right > region_x && region_bottom > region_y {
                    Some(library::runtime::frame::Region {
                        x: region_x as f64,
                        y: region_y as f64,
            ...
            if let Some(state) = &editor_context.interaction.preview.vector_editor_state {
                if let Some(id) = editor_context.selection.selected_entities.iter().next() {
                    if let Some(gc) = gui_clips.iter().find(|c| c.id() == *id) {
                        let renderer =
                            crate::panels::preview::vector_editor::renderer::VectorEditorRenderer {
                                state,
    ...
    for action in pending_actions {
        match action {
            PreviewAction::UpdateProperty {
                ...
                crate::widgets::property::update_property(
                    project_service,
                    comp_id,
                    track_id,
                    entity_id,
