...
use crate::panels::preview::{action::PreviewAction, clip::PreviewClip, gizmo};
...
pub(super) struct PreviewInteractions<'a> {
    ...
    pub(super) history_manager: &'a mut crate::command::history::HistoryManager,
...
impl<'a> PreviewInteractions<'a> {
    pub(super) fn new(
        ...
        history_manager: &'a mut crate::command::history::HistoryManager,
    ...
    pub(super) fn handle(
        ...
        if active_tool == crate::context::context_types::PreviewTool::Pan {
            return;
        }

        // 1. Gizmo Interaction
        let mut interacted_with_gizmo = false;
        if active_tool == crate::context::context_types::PreviewTool::Select {
        ...
        } else if active_tool == crate::context::context_types::PreviewTool::Shape {
            ...
            if ensure_loaded {
                    ...
                    .next()
                {
                    if let Some(gc) = self.gui_clips.iter().find(|c| c.id() == *id) {
                        // Build Transform
                        let transform = library::runtime::transform::Transform {
                            position: library::runtime::transform::Position {
                        ...
                        {
                            let mut interaction = crate::panels::preview::vector_editor::interaction::VectorEditorInteraction {
                                  state,
                                  transform,
                        ...
                        if changed {
                            ...
                            {
                                let new_path =
                                    crate::panels::preview::vector_editor::svg_writer::to_svg_path(
                                        state,
                                    );
        ...
        // 2. Hit Testing (Hover)
        let hovered_entity_id = if active_tool == crate::context::context_types::PreviewTool::Select
            || active_tool == crate::context::context_types::PreviewTool::Text
            || active_tool == crate::context::context_types::PreviewTool::Shape
        ...
        if !is_panning_input && !interacted_with_gizmo {
            ...
            if response.drag_started_by(PointerButton::Primary) {
                if let Some(hovered) = hovered_entity_id {
                    ...
                    let action =
                        crate::widgets::selection::SelectionAction::from_modifiers(&modifiers);
                    let track_id = self.get_track_id(hovered).unwrap_or_default();
                    let mut should_drag = true;

                    match action {
                        crate::widgets::selection::SelectionAction::Remove => {
                            if self.editor_context.is_selected(hovered) {
                                self.editor_context.toggle_selection(hovered, track_id);
                            }
                            should_drag = false;
                        }
                        crate::widgets::selection::SelectionAction::Add
                        | crate::widgets::selection::SelectionAction::Toggle => {
                            if !self.editor_context.is_selected(hovered) {
                                self.editor_context.toggle_selection(hovered, track_id);
                            }
                        }
    ...
    fn init_drag_state(&mut self, pointer_pos: Option<Pos2>) {
        if let Some(pointer_pos) = pointer_pos {
            ...
            self.editor_context.interaction.preview.body_drag_state =
                Some(crate::context::context_types::BodyDragState {
    ...
    fn handle_click_selection(&mut self, hovered_id: Option<Uuid>) {
        if self.editor_context.view.active_tool == crate::context::context_types::PreviewTool::Text
        {
        ...
        let action = crate::widgets::selection::get_click_action(&modifiers, hovered_id);
        ...
        match action {
            ...
            crate::widgets::selection::ClickAction::Clear => {
            ...
            crate::widgets::selection::ClickAction::DoNothing => {}
        }
    }
            ...
            .preview_selection_drag_start
        {
            if self.ui.input(|i| i.pointer.primary_down()) {
                ...
                if let Some(current_pos) = self.ui.input(|i| i.pointer.interact_pos()) {
                    ...
                    match crate::widgets::selection::get_box_action(&modifiers, found_clips) {
                        crate::widgets::selection::BoxAction::Replace(ids) => {
                            self.editor_context.selection.selected_entities.clear();
                            self.editor_context.selection.last_selected_entity_id = None;
                            self.editor_context.selection.last_selected_track_id = None;
                        ...
                        crate::widgets::selection::BoxAction::Add(ids) => {
                            let mut last_id = None;
                            let mut last_track = None;
                            for id in ids {
                                self.editor_context.selection.selected_entities.insert(id);
                        ...
                        crate::widgets::selection::BoxAction::Remove(ids) => {
                            for id in ids {
