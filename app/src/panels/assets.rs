...
pub(crate) fn assets_panel(
        ...
        ui.horizontal(|ui| {
                ...
                .clicked()
            {
                if let Some(paths) = rfd::FileDialog::new().pick_files() {
                    ...
                    let mut report = crate::context::context_types::ImportReport::default();
                                    ...
                                    row.col(|ui| {
                                        ui.push_id(comp.id, |ui| {
                                            ...
                                            response.context_menu(|ui| {
                                                use crate::widgets::context_menu::{ContextMenuBuilder, show_context_menu};
                                                    ...
                                                    match action {
                                                        ...
                                                        CompAction::Delete => {
                                                            if project_service.is_composition_used(comp.id) {
                                                                let mut dialog = crate::dialogs::confirmation::ConfirmationDialog::new();
                                                                dialog.open(
                                                                    "⚠ Confirm Composition Deletion",
                                                                    "This composition is used inside another timeline.\nDeleting it will remove all associated clips.\nAre you sure?",
                                                                    crate::dialogs::confirmation::ConfirmationAction::DeleteComposition(comp.id)
                                                                );
                                                        ...
                                                        AssetAction::Delete => {
                                                            if project_service.is_asset_used(asset.id) {
                                                                ...
                                                                dialog.open(
                                                                    ...
                                                                    crate::dialogs::confirmation::ConfirmationAction::DeleteAsset(asset.id)
                                                                );
                                                                editor_context.interaction.general.active_confirmation = Some(dialog);
                                                            } else {
    ...
    if let Some(report) = &editor_context.interaction.general.import_report {
        ...
        crate::widgets::modal::Modal::new("Import Result")
            .collapsible(false)
            .resizable(true)
            .open(&mut open)
            .show(ui.ctx(), |ui| {
