...
pub fn graph_editor_panel(
    ...
    let ruler_response = ui.interact(ruler_rect, ui.id().with("ruler"), Sense::click_and_drag());
    let graph_response = ui.interact(graph_rect, ui.id().with("graph"), Sense::click_and_drag());
    ...
    painter.line_segment([ruler_rect.left_bottom(), ruler_rect.right_bottom()], Stroke::new(1.0, Color32::BLACK));

    let to_screen_pos = |time: f64, value: f64| -> Pos2 {
         let x = graph_rect.min.x + editor_context.graph_editor.pan.x + (time as f32 * pixels_per_second);
    ...
    let end_time = ((graph_rect.width() - editor_context.graph_editor.pan.x) / pixels_per_second) as f64;
    ...
    let step_time = if step_time / step_base < 2.0 { step_base } else if step_time / step_base < 5.0 { step_base * 2.0 } else { step_base * 5.0 };
    ...
    for i in start_step..=end_step {
        ...
        let x = graph_rect.min.x + editor_context.graph_editor.pan.x + (t as f32 * pixels_per_second);
        
        if x >= graph_rect.min.x && x <= graph_rect.max.x {
             // Main Vertical Line
             painter.line_segment([Pos2::new(x, graph_rect.min.y), Pos2::new(x, graph_rect.max.y)], Stroke::new(1.0, Color32::from_gray(40)));
             
             // Ruler Tick & Label
             painter.line_segment([Pos2::new(x, ruler_rect.max.y), Pos2::new(x, ruler_rect.max.y - 10.0)], Stroke::new(1.0, Color32::GRAY));
             painter.text(
    ...
    let v_step_val = if v_step_val / v_step_base < 2.0 { v_step_base } else if v_step_val / v_step_base < 5.0 { v_step_base * 2.0 } else { v_step_base * 5.0 };
    ...
    for i in start_v..=end_v {
         ...
         if y >= graph_rect.min.y && y <= graph_rect.max.y {
             // Horizontal Line
             let color = if i == 0 { Color32::from_gray(80) } else { Color32::from_gray(40) };
             painter.line_segment([Pos2::new(graph_rect.min.x, y), Pos2::new(graph_rect.max.x, y)], Stroke::new(1.0, color));
        ...
        match property.evaluator.as_str() {
            "constant" => {
                if let Some(val) = property.value().and_then(|v| v.get_as::<f64>()) {
                     ...
                     painter.text(Pos2::new(graph_rect.min.x + 40.0, y - 5.0), egui::Align2::LEFT_BOTTOM, format!("{}: {:.2}", name, val), egui::FontId::default(), color);
            ...
            "keyframe" => {
                ...
                for (i, kf) in sorted_kf.iter().enumerate() {
                     ...
                     if !graph_rect.expand(10.0).contains(kf_pos) { continue; }
                     ...
                     point_response.context_menu(|ui| {
                         ...
                         ui.menu_button("Sine", |ui| {
                             if ui.button("Ease In").clicked() { action = Action::SetEasing(name.clone(), i, EasingFunction::EaseInSine); ui.close_menu(); }
                             if ui.button("Ease Out").clicked() { action = Action::SetEasing(name.clone(), i, EasingFunction::EaseOutSine); ui.close_menu(); }
                             if ui.button("Ease In Out").clicked() { action = Action::SetEasing(name.clone(), i, EasingFunction::EaseInOutSine); ui.close_menu(); }
                         });
                         ui.menu_button("Quad", |ui| {
                             if ui.button("Ease In").clicked() { action = Action::SetEasing(name.clone(), i, EasingFunction::EaseInQuad); ui.close_menu(); }
                             if ui.button("Ease Out").clicked() { action = Action::SetEasing(name.clone(), i, EasingFunction::EaseOutQuad); ui.close_menu(); }
                             if ui.button("Ease In Out").clicked() { action = Action::SetEasing(name.clone(), i, EasingFunction::EaseInOutQuad); ui.close_menu(); }
                         });
                         ui.menu_button("Cubic", |ui| {
                             if ui.button("Ease In").clicked() { action = Action::SetEasing(name.clone(), i, EasingFunction::EaseInCubic); ui.close_menu(); }
                             if ui.button("Ease Out").clicked() { action = Action::SetEasing(name.clone(), i, EasingFunction::EaseOutCubic); ui.close_menu(); }
                             if ui.button("Ease In Out").clicked() { action = Action::SetEasing(name.clone(), i, EasingFunction::EaseInOutCubic); ui.close_menu(); }
                         });
                         ui.menu_button("Quart", |ui| {
                             if ui.button("Ease In").clicked() { action = Action::SetEasing(name.clone(), i, EasingFunction::EaseInQuart); ui.close_menu(); }
                             if ui.button("Ease Out").clicked() { action = Action::SetEasing(name.clone(), i, EasingFunction::EaseOutQuart); ui.close_menu(); }
                             if ui.button("Ease In Out").clicked() { action = Action::SetEasing(name.clone(), i, EasingFunction::EaseInOutQuart); ui.close_menu(); }
                         });
                         ui.menu_button("Quint", |ui| {
                             if ui.button("Ease In").clicked() { action = Action::SetEasing(name.clone(), i, EasingFunction::EaseInQuint); ui.close_menu(); }
                             if ui.button("Ease Out").clicked() { action = Action::SetEasing(name.clone(), i, EasingFunction::EaseOutQuint); ui.close_menu(); }
                             if ui.button("Ease In Out").clicked() { action = Action::SetEasing(name.clone(), i, EasingFunction::EaseInOutQuint); ui.close_menu(); }
                         });
                         ui.menu_button("Expo", |ui| {
                             if ui.button("Ease In").clicked() { action = Action::SetEasing(name.clone(), i, EasingFunction::EaseInExpo); ui.close_menu(); }
                             if ui.button("Ease Out").clicked() { action = Action::SetEasing(name.clone(), i, EasingFunction::EaseOutExpo); ui.close_menu(); }
                             if ui.button("Ease In Out").clicked() { action = Action::SetEasing(name.clone(), i, EasingFunction::EaseInOutExpo); ui.close_menu(); }
                         });
                         ui.menu_button("Circ", |ui| {
                             if ui.button("Ease In").clicked() { action = Action::SetEasing(name.clone(), i, EasingFunction::EaseInCirc); ui.close_menu(); }
                             if ui.button("Ease Out").clicked() { action = Action::SetEasing(name.clone(), i, EasingFunction::EaseOutCirc); ui.close_menu(); }
                             if ui.button("Ease In Out").clicked() { action = Action::SetEasing(name.clone(), i, EasingFunction::EaseInOutCirc); ui.close_menu(); }
                         });
                         ui.menu_button("Back", |ui| {
                             if ui.button("Ease In").clicked() { action = Action::SetEasing(name.clone(), i, EasingFunction::EaseInBack); ui.close_menu(); }
                             if ui.button("Ease Out").clicked() { action = Action::SetEasing(name.clone(), i, EasingFunction::EaseOutBack); ui.close_menu(); }
                             if ui.button("Ease In Out").clicked() { action = Action::SetEasing(name.clone(), i, EasingFunction::EaseInOutBack); ui.close_menu(); }
                         });
                         ui.menu_button("Elastic", |ui| {
                             if ui.button("Ease In").clicked() { action = Action::SetEasing(name.clone(), i, EasingFunction::EaseInElastic); ui.close_menu(); }
                             if ui.button("Ease Out").clicked() { action = Action::SetEasing(name.clone(), i, EasingFunction::EaseOutElastic); ui.close_menu(); }
                             if ui.button("Ease In Out").clicked() { action = Action::SetEasing(name.clone(), i, EasingFunction::EaseInOutElastic); ui.close_menu(); }
                         });
                         ui.menu_button("Bounce", |ui| {
                             if ui.button("Ease In").clicked() { action = Action::SetEasing(name.clone(), i, EasingFunction::EaseInBounce); ui.close_menu(); }
                             if ui.button("Ease Out").clicked() { action = Action::SetEasing(name.clone(), i, EasingFunction::EaseOutBounce); ui.close_menu(); }
                             if ui.button("Ease In Out").clicked() { action = Action::SetEasing(name.clone(), i, EasingFunction::EaseInOutBounce); ui.close_menu(); }
                         });
    ...
    let x_cursor = graph_rect.min.x + editor_context.graph_editor.pan.x + (t_cursor as f32 * pixels_per_second);
    if x_cursor >= graph_rect.min.x && x_cursor <= graph_rect.max.x {
        painter.line_segment(
            [Pos2::new(x_cursor, graph_rect.min.y), Pos2::new(x_cursor, graph_rect.max.y)],
