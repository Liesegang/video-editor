...
pub fn handle_area_interaction(
    ...
    if response.hovered() {
         ...
         if ui.input(|i| i.modifiers.ctrl) && scroll_delta.y != 0.0 {
             ...
             let min_possible_zoom = content_rect.width() / (duration_sec as f32 * editor_context.timeline.pixels_per_second);
             ...
             if let Some(mouse_pos) = response.hover_pos() {
                 ...
                 editor_context.timeline.h_zoom =
                     (editor_context.timeline.h_zoom * zoom_factor).clamp(min_h_zoom_value, max_h_zoom_value);
             }
    ...
    response.context_menu(|ui| {
        let mut drop_in_frame = (editor_context.timeline.current_time * composition_fps as f32).round() as u64;
        ...
        if ui.button("Add Text Layer").clicked() {
            ...
            let text_clip = TrackClip::create_text("this is sample text", drop_in_frame, drop_out_frame);
