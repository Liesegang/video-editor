...
use library::model::project::{
    PortAddress, PortOwner, ProjectConnection, AUDIO_OUTPUT_PORT, IMAGE_OUTPUT_PORT,
    MERGE_IMAGES_PORT, MERGE_SOUNDS_PORT,
};
use library::model::{Clip, Composition, ListContent, Node, NodeContainer, Project};
...
fn variadic_pair_order_is_precomputed_before_hot_sort_comparisons() {
    ...
    let edges = super::graph::actual_node_edges(&fixture.project, &node_ids);
    let order = super::graph::SemanticNodeOrder::new(
        &fixture.project,
        &fixture.geometry,
        &node_ids,
        &edges,
...
fn edge_sort_is_an_unconditional_transitive_total_order() {
    ...
    for left in &edges {
        for middle in &edges {
            for right in &edges {
                let left_middle = super::graph::semantic_edge_cmp(&fixture.project, left, middle);
                let middle_right = super::graph::semantic_edge_cmp(&fixture.project, middle, right);
                if left_middle != Ordering::Greater && middle_right != Ordering::Greater {
                    assert_ne!(
                        super::graph::semantic_edge_cmp(&fixture.project, left, right),
                        Ordering::Greater,
                    );
                }
            }
    ...
    reversed.reverse();
    reversed.sort_by(|left, right| super::graph::semantic_edge_cmp(&fixture.project, left, right));
