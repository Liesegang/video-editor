...
pub(super) fn module_effect_controls(
    ...
    for parameter in &definition.interface.parameters {
        ...
        let mut reset = false;
        ui.horizontal(|ui| {
            ui.add_sized([112.0, 20.0], egui::Label::new(&parameter.name));
            let draft = state
                .inspector
                .effect_values
                .entry((attachment.id, draft_key))
                .or_insert_with(|| current.clone());
            if property_control(ui, draft, None, "", 0.1) {
    ...
    if additional_inputs.is_empty() {
        if definition.interface.parameters.is_empty() {
            ui.weak("Open the graph to add or publish controls.");
        }
        return;
    }
    ...
    for input in additional_inputs {
        ...
        ui.horizontal(|ui| {
            ui.add_sized([112.0, 20.0], egui::Label::new(&input.name));
    ...
    ui.weak("Bindings use published inputs; graph Node IDs remain private.");
}
