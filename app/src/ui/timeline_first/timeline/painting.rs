...
use super::geometry::{
    format_time, frame_for_seconds, major_tick_seconds, row_top, screen_x_to_seconds,
    seconds_to_screen_x, ROW_HEIGHT, RULER_HEIGHT,
};
...
pub(super) fn paint_background(
    ...
    for index in 0..rows.len() {
        let y = row_top(content_rect, state, index);
        let rect = Rect::from_min_size(
            Pos2::new(canvas_rect.min.x, y),
            Vec2::new(canvas_rect.width(), ROW_HEIGHT),
        );
...
pub(super) fn draw_ruler(
    ...
    for index in first..(first + count) {
        ...
        let x = seconds_to_screen_x(seconds, content_rect, state);
        painter.line_segment(
            [
...
pub(super) fn draw_playhead(
    ...
    let x = seconds_to_screen_x(seconds as f32, content_rect, state);
