...
use library::plugin::{PluginManager, PropertyDefinition, PropertyUiType, ExportSettings};
use library::service::{ExportService, RenderService, ProjectModel};
...
impl ExportDialog {
    ...
    fn show_configuration(
        ...
        ui.horizontal(|ui| {
             ...
             let current_comp_name = self.active_composition_id
             ...
             egui::ComboBox::from_id_salt("comp_select")
                 ...
                 .show_ui(ui, |ui| {
                     for comp in &project_read.compositions {
                         if ui.selectable_label(self.active_composition_id == Some(comp.id), &comp.name).clicked() {
        ...
        ui.horizontal(|ui| {
            ...
            let current_selection = self.selected_exporter_id.clone().unwrap_or_else(|| "Select...".to_string());
            
            egui::ComboBox::from_id_salt("exporter_select")
                .selected_text(current_selection)
                .show_ui(ui, |ui| {
                    for id in known_exporters {
                         if ui.selectable_label(self.selected_exporter_id.as_deref() == Some(id), id).clicked() {
        ...
        ui.horizontal(|ui| {
            ...
            egui::ComboBox::from_id_salt("export_range")
                ...
                .show_ui(ui, |ui| {
                    ui.selectable_value(&mut self.export_range, ExportRange::EntireComposition, "Entire Composition");
             ...
             if let Some(comp_id) = self.active_composition_id {
                 if let Some(comp) = project_read.compositions.iter().find(|c| c.id == comp_id) {
                     let (start, end) = match self.export_range {
                         ExportRange::EntireComposition => (0, (comp.duration * comp.fps).ceil() as u64),
                         ExportRange::WorkArea => (comp.work_area_in, comp.work_area_out),
                         _ => (0, 0),
                     };
                     ui.label(format!("Frames: {} to {} (Duration: {})", start, end, end.saturating_sub(start)));
                 }
             }
        }
        ...
        if let Some(exporter_id) = &self.selected_exporter_id {
            if let Some(definitions) = self.plugin_manager.get_export_plugin_properties(exporter_id) {
                egui::Grid::new("export_properties").num_columns(2).show(ui, |ui| {
                    for def in definitions {
                        ui.label(&def.label);
                        
                        let value = self.property_values.entry(def.name.clone()).or_insert(def.default_value.clone());
                        ...
                        match &def.ui_type {
                            ...
                            PropertyUiType::Integer { min, max, suffix } => {
                                if let PropertyValue::Number(n) = value {
                                     ...
                                     if ui.add(egui::Slider::new(&mut v, *min..=*max).text(suffix)).changed() {
                                    ...
                                    if let Ok(mut v) = s.parse::<i64>() {
                                        if ui.add(egui::Slider::new(&mut v, *min..=*max).text(suffix)).changed() {
                                            *value = PropertyValue::Number(ordered_float::OrderedFloat(v as f64));
                                        }
                                    }
                                }
                            }
                            PropertyUiType::Float { min, max, step, suffix } => {
                                 if let PropertyValue::Number(n) = value {
                                     let mut v = n.0;
                                     if ui.add(egui::Slider::new(&mut v, *min..=*max).step_by(*step).text(suffix)).changed() {
    ...
    fn load_defaults(&mut self, exporter_id: &str) -> Result<(), ()> {
         if let Some(defs) = self.plugin_manager.get_export_plugin_properties(exporter_id) {
             for def in defs {
    ...
    fn start_export(&mut self, project_lock: &Arc<RwLock<Project>>) {
         ...
         let target_comp_id = if let Some(id) = self.active_composition_id {
             ...
             let comp_index = match project_snapshot.compositions.iter().position(|c| c.id == target_comp_id) {
                 Some(idx) => idx,
                 None => {
             ...
             thread::spawn(move || {
                 ...
                 let project_model = match ProjectModel::new(Arc::new(project_snapshot.clone()), comp_index) {
                     Ok(pm) => pm,
                     Err(e) => {
                 ...
                 for (k, v) in &property_values_owned {
                      let json_val = match v {
                          ...
                          PropertyValue::Number(n) => serde_json::Value::Number(serde_json::Number::from_f64(n.0).unwrap()),
                 ...
                 let (start_frame, end_frame_total) = match export_range {
                     ExportRange::EntireComposition => (0, (composition.duration * composition.fps).ceil() as u64),
                 ...
                 while current_frame < end_frame_total {
                     ...
                     current_frame = end;
                     let pct = (current_frame.saturating_sub(start_frame)) as f32 / duration_frames as f32;
                     let _ = tx.send(pct);
                 }
