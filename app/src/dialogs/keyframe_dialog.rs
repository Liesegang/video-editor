...
pub fn show_keyframe_dialog(
    ...
    crate::widgets::modal::Modal::new("Edit Keyframe")
        .open(&mut open)
        .collapsible(false)
        .resizable(true)
        .show(ctx, |ui| {
                ...
                .show(ui, |ui| {
                    ...
                    ComboBox::from_id_salt("easing_selector")
                        ...
                        .show_ui(ui, |ui| {
                            ...
                            crate::widgets::easing_menus::show_easing_menu(
                                ui,
                                Some(&current_easing),
                                |easing| {
                                    state.easing = easing;
                ...
                if let (Some(_track_id), Some(entity_id)) = (state.track_id, state.entity_id) {
                    ...
                    let (base_name, component_suffix) = if state.property_name.ends_with(".x") {
                        (
                            ...
                            Some(crate::panels::graph_editor::PropertyComponent::X),
                        )
                    } else if state.property_name.ends_with(".y") {
                        (
                            state.property_name.trim_end_matches(".y"),
                            Some(crate::panels::graph_editor::PropertyComponent::Y),
                        )
                    } else {
                    ...
                    let target = if let Some((eff_idx, _)) = parse_key(base_name) {
                        ...
                        library::project::property::PropertyTarget::Clip
                    };
