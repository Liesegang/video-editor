    ...
    fn rasterize_ensemble_text(
        ...
        let mut layer = self.create_layer_surface()?;
        {
            ...
            for (start, ch) in text.char_indices() {
                let len = ch.len_utf8();
                let ch_str = &text[start..start + len];
                let (advance, _bounds) = font.measure_str(ch_str, None);

                // Store char data
                char_data.push((start, len, x_pos, advance));
                x_pos += advance;
            }
                    ...
                    DecoratorConfig::Backplate {
                        ...
                        match target {
                            BackplateTarget::Char => {
                                ...
                                for (i, (_start, _len, base_x, advance)) in
                                    char_data.iter().enumerate()
                                {
            ...
            for (i, (start, len, base_x, _advance)) in char_data.iter().enumerate() {
