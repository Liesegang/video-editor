...
impl SkiaRenderer {
    ...
    fn rasterize_ensemble_text(
        ...
        let mut layer = self.create_layer_surface()?;
        {
            ...
            // Optimization: Avoid allocation by storing slices and using char_indices
            let mut char_data = Vec::with_capacity(text.len());
            let mut x_pos = 0.0f32;

            let mut indices = text.char_indices().peekable();
            while let Some((i, _ch)) = indices.next() {
                let end = indices.peek().map(|(next_i, _)| *next_i).unwrap_or(text.len());
            ...
            for (i, (ch_str, base_x, _advance)) in char_data.iter().enumerate() {
                ...
                // Draw character
                // Optimization: Use the slice directly, no allocation
