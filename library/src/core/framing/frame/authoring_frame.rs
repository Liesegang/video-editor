...
use crate::core::render_plan::{PlannedSource, RenderPlan};
use crate::core::timeline_runtime::map_composition_time;
use crate::error::LibraryError;
use crate::model::BlendMode;
use crate::model::authoring::{
    AuthoringProject, ShapeKind, SourceRef, Timeline, TimelineId, TimelineItem,
};
...
pub fn evaluate_authoring_frame(
    ...
    if !plan.module_invocations.is_empty() {
        return Err(LibraryError::Render(
            "Module invocation execution is not connected to Timeline frame assembly".to_string(),
        ));
    }
    let root = project
        .timelines
    ...
    let items = collect_timeline_items(project, plan, root, time, &mut HashSet::new())?;
    Ok(FrameInfo {
        width: root.width,
        height: root.height,
...
mod tests {
    ...
    use crate::model::authoring::{AuthoringSession, SourceRef, TimelineInterval};
