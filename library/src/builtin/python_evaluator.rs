use crate::plugin::{EvaluationContext, Plugin, PropertyEvaluator, PropertyPlugin};
use crate::project::property::{Property, PropertyValue, Vec2};
use crate::runtime::color::Color;
use ordered_float::OrderedFloat;
use pyo3::ffi::PyThreadState; // Use PyThreadState from ffi
use pyo3::prelude::*;
use pyo3::types::{PyDict, PyDict as PyDictType, PyList, PyString, PyTuple};
use std::sync::Arc;
...
impl PropertyEvaluator for PythonExpressionEvaluator {
    fn evaluate(&self, property: &Property, time: f64, ctx: &EvaluationContext) -> PropertyValue {
        Python::with_gil(|py| {
            ...
            locals
                .set_item("time", time)
                .expect("Failed to set 'time' in Python locals");
            locals
                .set_item("frame", frame)
                .expect("Failed to set 'frame' in Python locals");
            locals
                .set_item("fps", fps)
                .expect("Failed to set 'fps' in Python locals");
            ...
            let result = PyModule::import(py, "builtins")
                ...
                .call1((
                    PyString::new(py, expression).into_any(),
                    py.None(),
                    Some(locals),
                ))
                .expect("Failed to call Python eval function");

            convert_pyobject_to_property_value(
                &result.extract().expect("Failed to extract Python result"),
            )
            .expect("Failed to convert Python object to PropertyValue")
        })
    }
}
...
fn expression_evaluator_python(_py: Python, m: &Bound<PyModule>) -> PyResult<()> {
    ...
    fn evaluate_expression(
        ...
        let result = PyModule::import(py, "builtins")?.getattr("eval")?.call1((
            PyString::new(py, &expression).into_any(),
            py.None(),
            Some(&locals),
        ))?;
...
fn convert_property_value_to_pyobject(py: Python, value: &PropertyValue) -> PyResult<PyObject> {
    match value {
        PropertyValue::Number(n) => Ok(n.into_inner().into_pyobject(py)?.unbind().into_any()),
        PropertyValue::Integer(i) => Ok(i.into_pyobject(py)?.unbind().into_any()),
        PropertyValue::String(s) => Ok(s.into_pyobject(py)?.unbind().into_any()),
        PropertyValue::Boolean(b) => Ok(b.into_pyobject(py)?.to_owned().unbind().into_any()),
        PropertyValue::Vec2(v) => {
            let x: PyObject = v.x.into_pyobject(py)?.unbind().into_any();
            let y: PyObject = v.y.into_pyobject(py)?.unbind().into_any();
            Ok(PyTuple::new(py, [x, y])?.unbind().into_any())
        }
        PropertyValue::Color(c) => {
            let r: PyObject = c.r.into_pyobject(py)?.unbind().into_any();
            let g: PyObject = c.g.into_pyobject(py)?.unbind().into_any();
            let b: PyObject = c.b.into_pyobject(py)?.unbind().into_any();
            let a: PyObject = c.a.into_pyobject(py)?.unbind().into_any();
            Ok(PyTuple::new(py, [r, g, b, a])?.unbind().into_any())
        }
...
fn convert_pyobject_to_property_value(obj: &Bound<'_, PyAny>) -> PyResult<PropertyValue> {
    ...
    } else if let Ok(dict) = obj.extract::<Bound<'_, PyDictType>>() {
        // Using aliased type
