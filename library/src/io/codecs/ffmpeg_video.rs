use crate::extensions::traits::{LoadPlugin, LoadRequest, LoadResponse, Plugin};
use crate::cache::CacheManager;
use crate::error::LibraryError;
use crate::io::video::{MediaProbe, VideoReader};
...
impl LoadPlugin for FfmpegVideoLoader {
    ...
    fn get_asset_kind(&self, path: &str) -> Option<crate::core::model::asset::AssetKind> {
        ...
        match ext.as_str() {
            "mp4" | "mov" | "avi" | "mkv" | "webm" => {
                Some(crate::core::model::asset::AssetKind::Video)
            }
            "mp3" | "wav" | "aac" | "flac" | "ogg" => {
                Some(crate::core::model::asset::AssetKind::Audio)
            }
            _ => None,
        }
    }
    ...
    fn get_metadata(&self, path: &str) -> Option<crate::extensions::traits::AssetMetadata> {
        let ext = std::path::Path::new(path)
            .extension()?
            .to_str()?
            .to_lowercase();
        ...
        // Check if cached
        {
            ...
            if let Some(reader) = readers.get(path) {
                return Some(crate::extensions::traits::AssetMetadata {
                    kind: crate::core::model::asset::AssetKind::Video,
