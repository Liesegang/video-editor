...
impl EffectPlugin for PixelSorterPlugin {
    fn apply(
        &self,
        input: &crate::graphics::renderer::RenderOutput,
        params: &HashMap<String, PropertyValue>,
        gpu_context: Option<&mut crate::graphics::skia_utils::GpuContext>,
        ...
        let image = match input {
            crate::graphics::renderer::RenderOutput::Image(img) => img.clone(),
            crate::graphics::renderer::RenderOutput::Texture(info) => {
                if let Some(ctx) = gpu_context {
                    let sk_image = crate::graphics::skia_utils::create_image_from_texture(
        ...
        Ok(crate::graphics::renderer::RenderOutput::Image(Image {
            width: image.width,
            height: image.height,
            data: processed_data,
        }))
