...
fn compressed_chunk_preroll_prevents_artificial_boundary_silence() -> Result<()> {
    ...
    for index in [0, 3, 7] {
        ...
        for channel in 0..2 {
            ...
            let mut local = before.samples()[tail_start..]
                .chunks_exact(2)
            ...
            let largest_local_step = local
                .windows(2)
                .map(|pair| (pair[1] - pair[0]).abs())
                .fold(0.0_f32, f32::max);
            assert!(
                boundary_step <= largest_local_step + f32::EPSILON,
                "compressed chunk {index}->{next} channel {channel} introduced boundary step {boundary_step}, local max {largest_local_step}",
                next = index + 1,
            );
        }
    }
