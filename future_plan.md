...
- [ ] **部分実装：Transition を Timeline の第一級モデルとして実装する。** Timeline 所有の型付き authoring model、atomic edit API、validation、局所 invalidation、階層型 RenderPlan、Image/Audio の実レンダリング、Inspector、有限な Transition Module の production Node Editor 編集、Published input/parameter/automation、Nested InstancePath ごとの override、Inspector/Dope Sheet/Curve Editor authoring、native HTTP QA は実装済み。preset の drag/drop、再利用 Transition Template の作成・管理と必須追加 input の atomic 割当 UI、Timeline handle、source handle 診断、ripple/roll 等との編集 tool 連携、非 Normal Blend Mode の合成意味論、追加 built-in preset、広い format/rate 条件の golden は残る。
