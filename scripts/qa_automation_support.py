"""Shared native-UI discovery for Timeline and Curve automation surfaces."""

from qa_support import QaFailure, activate_dock_tab, bring_timeline_component
...
def wait_item_automation_surfaces(
    ...
    activate_dock_tab(
        client, TIMELINE_TAB, "Timeline", description + " Dope Sheet"
    )
    ...
    activate_dock_tab(
        client, CURVE_TAB, "Curve Editor", description + " Curve Editor"
    )
...
def require_exact_item_times(surfaces, expected_local_times, expected_timeline_times):
    ...
    for index, (local_time, timeline_time) in enumerate(
        ...
        if abs(float(timeline_metadata.get("timeline_time", -1.0)) - timeline_time) > 0.001:
