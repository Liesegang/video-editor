...
pub fn node_editor_panel(
    ...
    let mut parameter_host = super::parameter::NodeParameterHost {
        project,
        service,
        instance,
    ...
    for action in actions {
        match action {
            ModuleEditorAction::PublishParameterKeyframe { target } => {
                ...
                match result {
                    Ok(published) => {
                        set_active_definition(&mut state.node_editor, published.definition_id);
                        state.status = "Added Timeline keyframe".to_string();
                    }
