...
pub(super) struct PublicationSpec {
...
pub(super) fn publication_icon(
    ...
    let existing = project.timelines.get(&timeline_id).and_then(|timeline| {
        timeline
            ...
            .iter()
            .find(|parameter| parameter.target == spec.target)
    });
    let response = ui
        .add(
    ...
    egui::CollapsingHeader::new("Instance controls")
        ...
        .show(ui, |ui| {
            ...
            for parameter in &timeline.published_parameters {
                    ...
                    .horizontal(|ui| {
                        ...
                        let (finished, edited_value) = {
                            let value = state
                                ...
                                .entry(draft_key)
                                .or_insert(initial);
                            let finished = super::property_control(
                                ui,
                                &control_id,
                                value,
                                None,
                ...
                super::value_provenance(ui, false, overridden);
            }
