...
impl SkiaRenderer {
        ...
        #[cfg(feature = "gl")]
        {
            if target_sizes.is_empty() {
                return Err(LibraryError::Render(
                    "GPU Point preflight requires at least one reached render target".to_string(),
                ));
            }
            if !self.group_surfaces.is_empty() || !self.retained_group_surfaces.is_empty() {
                return Err(LibraryError::Render(
                    "GPU Point preflight requires an idle renderer with no open groups".to_string(),
                ));
            }
            if self.surface_contract.working().is_none() {
                return Err(LibraryError::Render(
                    "GPU Point export preflight requires the Project-linear surface contract"
                        .to_string(),
                ));
            }
            self.activate_graphics_context()?;
            ...
            let (gpu_context, scene_runtime) = (
                self.gpu_context.as_mut().ok_or_else(|| {
                    LibraryError::Render(
                        "GPU Point preflight has no active Ganesh context".to_string(),
                    )
                })?,
                self.scene_runtime.as_mut().ok_or_else(|| {
                    LibraryError::Render(
                        "GPU Point preflight has no SceneRuntime for the active context"
                            .to_string(),
                    )
                })?,
            );
    ...
    pub(super) fn rasterize_point_output(
        &mut self,
        ...
        #[cfg(feature = "gl")]
        {
            ...
            let scene_image = self.point_scene_image(request)?;
    ...
    pub(super) fn draw_point_output(
        &mut self,
        request: PointRasterRequest<'_>,
        opacity: f64,
    ...
    fn point_scene_image(
        ...
        let scene_texture = {
            let gpu_context = self.gpu_context.as_mut().ok_or_else(|| {
                LibraryError::Render(
                    "GPU Point unavailable: SkiaRenderer has no active GPU context".to_string(),
                )
            })?;
            ...
            let scene_runtime = self.scene_runtime.as_mut().ok_or_else(|| {
                LibraryError::Render(
                    "GPU Point unavailable: SceneRuntime was not created for the active GPU context"
                        .to_string(),
                )
            })?;
        ...
        let gpu_context = self.gpu_context.as_mut().ok_or_else(|| {
            LibraryError::Render(
                "GPU Point lost its GPU context before Ganesh ingestion".to_string(),
            )
        })?;
