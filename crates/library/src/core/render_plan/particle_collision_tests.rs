...
use crate::model::frame::particle::{PARTICLE_MAX_COLLIDERS, ParticleCollider};
...
fn add_collision(definition: &mut crate::model::authoring::ModuleDefinition) -> uuid::Uuid {
    let node = Node::new_catalog_node(ParticleNodeRole::CollisionPlane.catalog_id())
...
fn repeated_collisions_compile_after_forces_in_authored_order_and_respect_the_bound() {
    ...
    for (count, expected) in [
        ...
        for _ in 0..count {
            collisions.push(add_collision(&mut definition));
        }
    ...
    let collision = add_collision(&mut wrong_order);
